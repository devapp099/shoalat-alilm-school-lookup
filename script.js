/* ============================================================
   script.js
   منطق البحث عن بيانات الطلبة - مدرسة شعلة العلم
...
(function () {
  ...
  function normalizeText(text) {
    ...
    // قبول الاسم مع كلمتي «بن» و«بنت» أو بدونهما، وتوحيد كتابة عبدالله.
    t = t.replace(/(^|\s)(بن|بنت)(?=\s|$)/g, " ");
  ...
  function performSearch() {
    ...
    if (!nameValue || !nameValue.trim()) {
      showMessage("يرجى إدخال اسم الطالب أو الطالبة.", "");
    ...
    window.setTimeout(async () => {
      try {
        ...
        if (matches.length === 0) {
          failedAttempts += 1;
          showMessage(
            "عذرًا، لم يتم العثور على اسم مطابق. يرجى التأكد من كتابة الاسم كما ورد في السجل المدرسي.",
            ""
          );
          markFieldError(nameInput, true);
          ...
          showMessage(
            "يوجد أكثر من طالب أو طالبة يتطابق مع هذا الاسم. يرجى إضافة جزء آخر من الاسم لتحديد المطلوب.",
            "info"
          );
          nameInput.focus();
        } else {
  ...
  if (!SpeechRecognition) {
    ...
    recognition.onstart = () => {
      ...
      voiceHint.textContent = "جارٍ الاستماع... انطق اسم الطالب أو الطالبة الثلاثي بوضوح.";
    };
